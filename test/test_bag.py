...
class BagTestCase(QCGCTest):
    ...
    def test_bag_remove(self):
        ...
        # Remove last element
        b = lib.qcgc_bag_remove(b, ffi.cast("void *", 9))
        ...
        b = lib.qcgc_bag_remove(b, ffi.cast("void *", 5))
        self.assertEqual(b.count, 8)

        # Check whether other elements are still there
        ...
        # Bag with size 1
        b = lib.qcgc_bag_create(1)
        b = lib.qcgc_bag_add(b, ffi.NULL)

        b = lib.qcgc_bag_remove(b, ffi.NULL)
        self.assertEqual(b.count, 0)
    ...
    def test_bag_remove_index(self):
        ...
        # Remove last
        b = lib.qcgc_bag_remove_index(b, b.count - 1)
        self.assertEqual(b.count, 9)

        # Remove other
        b = lib.qcgc_bag_remove_index(b, 0)
        self.assertEqual(b.count, 8)
    ...
    def test_bag_shrink(self):
        b = lib.qcgc_bag_create(12)
        for i in range(3):
            b = lib.qcgc_bag_add(b, ffi.cast("void *", i))

        self.assertEqual(b.size, 12)
        b = lib.qcgc_bag_remove_index(b, 2)
